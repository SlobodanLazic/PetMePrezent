<?php
    ...
    $json = json_encode($arrayofEmails);
    echo $json;
    if(isset($_POST["submit"]))
    {
        echo $result;
        if ($result != "Vi ste se već prijavili na naš newletter sa ovom email adresom" 
        && $result == "Uspešno ste uneli email i prijavili se na naš newslettter!")
        {
    ...
    /*
    $url = "http://localhost/PetMePrezent/public/#";
    $curlinitialize = curl_init($url);
    
    curl_setopt($curlinitialize, CURLOPT_SSL_VERIFYPEER, false);
    curl_setopt($curlinitialize, CURLOPT_RETURNTRANSFER, true);

    $responseJSON = curl_exec($curlinitialize);
